...
class MainWindow:
    ...
    def __init__(self):
        ...
        self.add_files([uri if not uri.startswith("file://") else
                        GLib.filename_from_uri(uri)[0] for uri in sys.argv[1:]])
            ...
            try:
                for rename in self.renames:
                    ...
                    if not os.path.exists(dst_file):
                        ...
                        if self.cfg["allow-revert"]:
                            self.add_revert(src_file, dst_file, os.path.basename(src_file), os.path.basename(dst_file))
                        self.rename_count += 1
            finally:
    ...
    def update_renames(self, widget=None):
        ...
        start_index = self.start_index_label_spin.get_value_as_int()
        is_reg_ex = self.reg_ex_button.get_active()
        include_ext = self.include_ext_button.get_active()
        before = self.find_entry.get_text()
        after = self.replace_entry.get_text()
